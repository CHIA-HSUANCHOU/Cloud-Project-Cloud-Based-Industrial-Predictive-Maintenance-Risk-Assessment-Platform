...
def page2():
    ...
    # --- Next step button (enabled only when likely failure) ---
    #if st.button("Next step → / 進入五種 failure 分析 →", disabled=(not likely_failure)):
...
def page3():
    ...
    # 5 條 pipeline ai4i_xgb_pipeline_final
    # FAILURES = {
    #     "Tool wear failure": "ai4i_tool_wear_pipeline.pkl",
    #     "Heat dissipation failure": "ai4i_heat_dissipation_pipeline.pkl",
    #     "Power failure": "ai4i_power_failure_pipeline.pkl",
    #     "Overstrain failure": "ai4i_overstrain_pipeline.pkl",
    #     "Random failure": "ai4i_random_failure_pipeline.pkl",
    # }

    FAILURES = {
        "Tool wear failure": "ai4i_xgb_pipeline_final.pkl",
        "Heat dissipation failure": "ai4i_xgb_pipeline_final.pkl",
        "Power failure": "ai4i_xgb_pipeline_final.pkl",
        "Overstrain failure": "ai4i_xgb_pipeline_final.pkl",
        "Random failure": "ai4i_xgb_pipeline_final.pkl",
    }
    ...
    probs = {}
    for name, pkl in FAILURES.items():
        pipe = load_pipeline(pkl)
        probs[name] = float(pipe.predict_proba(user_df)[0, 1])

    cols = st.columns(5)
    for i, (name, proba) in enumerate(probs.items()):
        with cols[i]:
            render_risk_card(name, proba)
    ...
    # =========================
    st.markdown("## 🔎 Why this result? (AI Explanation) / 風險分析")
    st.caption("Waterfall 了解單筆如何累積到預測；Force 看推拉（之後補）。")

    failure_names = list(FAILURES.keys())
    failure_labels = [FAILURE_SHORT.get(n, n) for n in failure_names]

    explain_failure_tabs = st.tabs(failure_labels)

    for tab, failure_name in zip(explain_failure_tabs, failure_names):
        with tab:
            pkl = FAILURES[failure_name]
            cache_key = f"artifacts_{failure_name}"
    ...
    # =========================
    st.markdown("## 📊 Global Explanation / 模型整體解釋")
    st.caption("Beeswarm：每個點是一筆背景資料；Bar：平均重要性。")

    global_failure_tabs = st.tabs(failure_labels)

    for tab, failure_name in zip(global_failure_tabs, failure_names):
        with tab:
            cache_key = f"artifacts_{failure_name}"

            # 確保 artifacts 存在（如果使用者直接滑到最下面）
            if cache_key not in st.session_state:
                pipe = load_pipeline(FAILURES[failure_name])
                proba, exp_user, exp_bg = compute_one_failure_artifacts(pipe, user_df, bg_df)
                st.session_state[cache_key] = {
                    "proba": float(proba),
                    "exp_user": exp_user,
            ...
            with tab_bar:
                fig = plt.figure()
                shap.plots.bar(exp_bg, max_display=15, show=False)
                st.pyplot(fig, clear_figure=True)
